...
| Any other thread (background song/stage loaders, tasks) | `AppDomain.CurrentDomain.UnhandledException` → `CrashLog.Write(ex, "UnhandledException")` |
| A managed exception thrown inside a native→managed callback (CADisplayLink, touch handlers, `UIAlertAction`, `NSTimer`, …) as it crosses back into native code | `ObjCRuntime.Runtime.MarshalManagedException` → `CrashLog.Write(ex, "MarshalManagedException")` |

The last hook matters: an exception thrown in a native callback is marshaled to an Objective-C
`NSException` by Xamarin (`xamarin_unhandled_exception_handler` → `abort()`), a path that does
**not** fire `AppDomain.UnhandledException`. Without it such a crash leaves only an uninformative
`SIGABRT` and no managed stack. (It overlaps `OnFrame`'s `try/catch` for render-thread throws —
the resulting duplicate `.log` is harmless.)
...
- Everything user-facing goes to `Documents/CrashLogs/`, surfaced to the Files app via
  `UIFileSharingEnabled`. Reports are **never deleted**.
- Managed: `crash_<ts>_<source>.log` (one per crash, millisecond timestamp).
- Native: the handler writes a fixed **pending file in `Library/Caches`** (`crash_native_pending.crash`),
  *not* in the shared dir. On the next launch `PrepareNativeReport` **promotes** it into
  `Documents/CrashLogs/crash_<ts>_native.crash` **iff it is non-empty**, then re-creates the empty
  pending file (the handler opens `O_WRONLY|O_TRUNC` and can't create/rename safely from signal
  context). Keeping the pending stub in Caches means testers **never see an empty `.crash`** — the
  shared dir only ever gains a native report when a fault was actually captured.
- `crash_native_status.txt` — a breadcrumb written at `Install()` recording that handlers armed,
  the alt-stack/prefix state, and image count. So an **empty `CrashLogs` after a crash** positively
  reads as an *uncatchable OS kill* (OOM/jetsam, watchdog, `SIGKILL`) rather than "the handler
  silently failed to install".
- `CrashLog.FlushPreviousCrashLogs()` (called once on launch) echoes retained reports to the device
  console (visible in Console.app / Xcode).
