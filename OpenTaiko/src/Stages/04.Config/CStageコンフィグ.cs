	...
	public override int Draw() {
		...
		*/
		...
		// 曲データの一覧取得中は、キー入力を無効化する
			...
			if (OpenTaiko.Pad.bPressed(EInstrumentPad.Drums, EPad.SD) || OpenTaiko.Pad.bPressed(EInstrumentPad.Drums, EPad.LBlue) || OpenTaiko.Pad.bPressed(EInstrumentPad.Drums, EPad.LeftChange)) {
				this.tカーソルを上へ移動する();
			}
			...
			if (OpenTaiko.Pad.bPressed(EInstrumentPad.Drums, EPad.LT) || OpenTaiko.Pad.bPressed(EInstrumentPad.Drums, EPad.RBlue) || OpenTaiko.Pad.bPressed(EInstrumentPad.Drums, EPad.RightChange)) {
				this.tカーソルを下へ移動する();
			}
		}
		return 0;
