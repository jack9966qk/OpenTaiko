...
";
	...
	protected void Crash(Exception exception) {
		bCrashed = true;
		// iOS: surface Lua errors to the device console (os_log) — they otherwise only hit the on-screen
		// overlay, which is hard to read. A crashed script stops updating (a frozen "softlock" on a stage).
