...
internal class OpenTaiko : Game {
	...
	/// app auto-walks through first-time setup / title / song-select without manual taps. MUST be false for
	...
	public void MountActivity(CActivity? Stage) {
		if (Stage == null)
			return;
		// Guarantee the stage's resources exist BEFORE Activate(), idempotently. Activities use those
		// resources inside their own Activate() (e.g. fonts: CActResultSongBar.pfMusicName,
		// CStageConfig.ftFont), so creating them after Activate() NREs. The created-flags make this safe
		// regardless of whether the PreAssetsLoading startup pass already created them (or partially failed).
