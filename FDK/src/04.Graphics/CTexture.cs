...
public partial class CTexture : IDisposable {   // streaming subsystem is in CTexture.Streaming.cs
	...
	public void MakeTexture(string strFileName, bool bBlackTransparent) {
		...
		// Lazy upload (0.6.0 behavior): read just the dimensions now (cheap header read) so layout that reads
		// szImageSize/szTextureSize before drawing keeps working, then defer the pixel decode + GL upload to
		// first draw (tCacheOnDraw). Undrawn textures never reach GPU memory — this is the main memory win over
		// uploading every loaded texture eagerly. Falls through to an eager decode if the header read fails.
		try {
	...
	// ── iOS texture memory cache (restored 0.6.0 LRU, adapted to the 0.6.1 streaming model) ──────────
	// Live GPU texture memory: bytes uploaded via GenTexture, released on Dispose/eviction. iOS jetsam is
	// driven by native memory and the skin's textures dominate it. Streaming does the first upload; under
	// memory pressure the least-recently-drawn file-backed textures free their GL handle (keeping the source
	// path) and re-decode on their next draw, so eviction is transparent.
	...
	// At the top of every draw: stamp the LRU time, and upload if this texture is deferred — either its
	// first draw (lazy upload, 0.6.0 behavior: undrawn textures never reach GPU memory) or after eviction.
